/**
 * Gemini 协议桥接
 * 实现 ProtocolAdapter，内部委托给 GeminiAdapter
 */
...
export class GeminiProtocolBridge implements ProtocolAdapter {
  ...
  async *call(args: BaseCallArgs): AsyncIterable<UnifiedStreamEvent> {
    ...
    try {
      const modelConfigs = await loadModelConfigsForProvider('google');
      ...
      logger.error('Gemini protocol bridge error', error);
