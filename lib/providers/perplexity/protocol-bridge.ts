 ...
 */
export class PerplexityProtocolBridge implements ProtocolAdapter {
  ...
  async *call(args: BaseCallArgs): AsyncIterable<UnifiedStreamEvent> {
    ...
    try {
      // 查找模型配置
      const modelConfig = perplexityModelConfigs.find(m => m.id === model);
      if (!modelConfig) {
      ...
      logger.error('Perplexity protocol bridge error', error);
      yield {
        type: 'error',
